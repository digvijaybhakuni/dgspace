(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('jhMonoApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-jhMonoApp-error');
                var entityKey = httpResponse.headers('X-jhMonoApp-params');
                if (errorHeader) {
